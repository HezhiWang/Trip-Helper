...
class overview_plot:
    ...
    def plot_bar_chart(self, filename):
        ...
        if (filename == 'hotel'):
            ...
            ind = np.arange(12)
            width = 0.35
            rects1 = axarr[0].bar(ind, df_mean['Total_review'], width, color='r', label = 'Total reviews', alpha = 0.7)
            axarr[0].set_xticks(ind + width)
            axarr[0].legend(bbox_to_anchor=(0.70, 1.05), loc=2, borderaxespad=0.)
            ...
            rects2 = axarr[1].bar(ind, df_mean['Avgscore'], width, color='b', label = 'Average Score', alpha = 0.7)
            axarr[1].set_xticks(ind + width)
            axarr[1].legend(bbox_to_anchor=(0.70, 1.05), loc=2, borderaxespad=0.)
